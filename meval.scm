...
;(set! *unparser-list-depth-limit* 5)
;(set! *unparser-list-breadth-limit* 5)
(define true #t)
(define false #f)
...
(define (eval-if exp env)
  ;(display "eval-if: ")
  ;(display exp)
  ;(display "\n    (if-predicate exp):") (display (if-predicate exp))
  ;(display "\n    (if-consequent exp):") (display (if-consequent exp))
  ;(display "\n    (if-alternative exp):") (display (if-alternative exp))
  ;(display "\n")
  (if (true? (meval (if-predicate exp) env))
      (let (( a (meval (if-consequent exp) env)))
        ;(display "aaa:")
        ;(display a) 
        ;(display " :eval exp=")
        ;(display (if-consequent exp))
        ;(display "\n")
        a
        )
...
(define (if-consequent exp) 
  ;(display "if-consequent: ")
  ;(display (caddr exp))
  ;(display "\n")
  (caddr exp))

(define (if-alternative exp)
  ;(display "if-alternative: ")
  ;(display exp)
  ;(display " test:")
  ;(display (cdddr exp))
  ;(display  "re:")
...
(define primitive-procedures
  (list (list 'car car)
	...
	(list 'display display)
        ...
        (list 'list list)
        (list 'length length)
        (list 'print-dot (lambda (x) (print-dot x) ))
        (list 'cond 'cond)
        (list 'trace trace)
...
;(trace 'primitive-procedure? )
;(sys:set-log-level 7)
;(text-of-quotation (read))
(start-meval)